 ...
 *			                                           stores it in string Txt
 *			           void display()				            :	displays the original and the changed word
 */
// ------------------------------------{SHOW ME THE CODE}----------------------------------------
 import java.io.*;
 class Rearrange{
    String Txt, Cxt; int len; BufferedReader br=new BufferedReader(new InputStreamReader(System.in));
    Rearrange(){
      Txt=""; Cxt=""; len=0;
    }  
    public void readWord()throws IOException{
     	System.out.println("Enter the word:"); Txt=br.readLine().toUpperCase();
    }
    public void convert(){
      int i,x=0; len=Txt.length()-1;
      for(i=0;i<=len;i++){
         switch(Txt.charAt(i)){
           case 'A':
           case 'E':
      ...
      if (x==0)
        ...
        Cxt=Txt.substring(i); Cxt=Cxt+Txt.substring(0,i)+"C";
      }
    }
    void display(){
   ...
   public static void main(String args[]) throws Exception{
   		Rearrange one=new Rearrange(); one.readWord(); one.convert(); one.display();
   }
 }
// ------------------------------------{END OF THE CODE}-----------------------------------------